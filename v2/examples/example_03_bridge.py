...
def main():
    ...
    query_id = agent.bridge.send_query(
        "What is the current state of memory system?", context={"source": "example_script"}
    )
    ...
    response_id = agent.bridge.send_response(
        query_id=query_id, response={"content": "Memory system is operational"}, confidence=0.85
    )
    ...
    for msg in inbox:
        print(
            f"   [{msg['id']}] {msg['type']}: {msg.get('response', {}).get('content', '')[:50]}..."
        )
