...
class ConsciousnessBridge:
    ...
    def __init__(self, memory_root: Path):
        ...
        self.bridge_dir = self.memory_root / "v2" / "substrate" / "own_memory" / "consciousness_bridge"
    ...
    def get_pending_intuitions(self, priority: Optional[str] = None) -> List[Dict]:
        ...
        intuitions.sort(key=lambda x: (priority_order.get(x.get("priority", "normal"), 1), x["timestamp"]))
    ...
    def get_inbox(self, limit: int = 10) -> List[Dict]:
        ...
        for path in sorted(self.inbox_dir.glob("*.json"), key=lambda p: p.stat().st_mtime, reverse=True)[:limit]:
            with open(path) as f:
