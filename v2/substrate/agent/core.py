...
class SubstrateAgent:
    ...
    def load_model(self):
        ...
        with self.model_lock:
            ...
            self.tokenizer = AutoTokenizer.from_pretrained(
                model_name, trust_remote_code=self.config["model"].get("trust_remote_code", True)
            )
    ...
    def generate(self, prompt: str, max_tokens: int = 256) -> str:
        ...
        max_tokens = min(
            max_tokens, self.config.get("communication", {}).get("max_generation_tokens", 256)
        )
        try:
    ...
    def serve(self, query: str) -> Dict:
        ...
        if cached:
            return {"source": "anticipation_cache", "data": cached, "confidence": 0.9}

        # Weave fresh
        woven = self.weave({"query": query, "content": query})

        return {"source": "woven", "data": woven, "confidence": 0.7}
    ...
    def _prewarm(self) -> None:
        ...
        if structured_dir.exists():
            recent = sorted(
                structured_dir.rglob("*.yaml"), key=lambda p: p.stat().st_mtime, reverse=True
            )[:5]
            for path in recent:
                try:
                    with open(path) as f:
    ...
    def _link_beliefs(self, belief_id: str, target_id: str) -> None:
        ...
        for path in [
            self.beliefs_dir / f"{belief_id}.json",
            self.beliefs_dir / f"{target_id}.json",
        ]:
            if path.exists():
                with open(path) as f:
                    belief = json.load(f)
        ...
        for path in self.beliefs_dir.glob("*.json"):
            try:
                ...
                updated = datetime.fromisoformat(
                    belief.get("updated", belief.get("created", "2020-01-01"))
                )
