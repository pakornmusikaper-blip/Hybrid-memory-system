            ...
            for path in structured_dir.rglob("*.yaml"):
                try:
                    ...
                    if data.get("type") == "fact" or data.get("category") == "decisions":
                        facts.append({
                            "statement": data.get("summary", str(data)),
                            "source": str(path)
                        })
                except:
                    pass
        ...
        for path in self.beliefs_dir.glob("*.json"):
            ...
            if last_validated:
                ...
                if last_date < cutoff:
                    ...
                    belief["confidence"] = new_confidence
                    belief["status"] = "decaying" if new_confidence < 0.3 else belief.get("status", "active")
        ...
        for i, b1 in enumerate(beliefs):
            for b2 in beliefs[i+1:]:
                if self._is_contradiction(
                    b1.get("statement", "").lower(),
                    b2.get("statement", "").lower()
                ):
